...
namespace Common.Utils.Tools.Models
{
    public class ObjectResult<T> : ObjectResult
    {
        ...
        public ObjectResult(HttpResponse<T> response)
            : base(response) // aquí se asigna a Value internamente
        {
            Response = response;
            StatusCode = (int)response.StatusCode; // sincronización explícita
            Value = response;                 // contrato fuerte en la salida
        }
    }
}
